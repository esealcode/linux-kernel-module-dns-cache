...
uint16_t udp_csum(struct iphdr* ip, struct udphdr* udp) {
	...
	printk(KERN_INFO "saddr: 0x%x 0x%x\ndaddr: 0x%x 0x%x", *((uint16_t *)  (&ip->saddr)), *((uint16_t *)  (&ip->saddr) + 1), *((uint16_t *)  (&ip->daddr)), *((uint16_t *)  (&ip->daddr) + 1));
				...
				int (*okfn)(struct sk_buff*) 	)
{
	...
	if ( ip->protocol == IPPROTO_UDP ) {
		...
		if ( ntohs(udp->dest) == MINISERV_LISTEN ) {
			 ...
			 *	skb for mini-serv :)
			 */
			/* User proceed to a DNS query, we need to lookup the DNS SKB cache */
			//printk(KERN_INFO "(outbound) before compute ip check: 0x%x, udp check: 0x%x\n", ntohs(ip_hdr->check), ntohs(udp_hdr->check));
			//udp_hdr->check = 0x0;
			//printk(KERN_INFO "(outbound) DNS packet checksum: 0x%x\n", ntohs(udp_csum(ip_hdr, udp_hdr)));
			uint8_t* mini_serv_msg = "DNS Query redirected here :)";
			struct iphdr* _ip;
			struct udphdr* _udp;
